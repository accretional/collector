...
// Package sqliteext provides a CGo-based SQLite driver that supports loading
// native extensions. This is needed because pure Go SQLite implementations
// (like modernc.org/sqlite) cannot load C extensions.
//
...
// use modernc.org/sqlite instead.
//
// Example usage:
//
//	db, err := sqliteext.Open(ctx, "/path/to/db", "/path/to/extension.so", "extension_init")
//	if err != nil {
//	    return err
//	}
//	defer db.Close()
//
//	rows, err := db.QueryContext(ctx, "SELECT * FROM virtual_table WHERE ...")
package sqliteext
