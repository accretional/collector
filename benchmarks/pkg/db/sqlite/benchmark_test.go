...
// getScenario gets the benchmark scenario from environment variable
func getScenario() string {
	scenario := os.Getenv("BENCHMARK_SCENARIO")
	if scenario == "" {
		return "hybrid_cgo"
	}
	return scenario
...
func setupBenchmarkStore(tb testing.TB) (collection.Store, func()) {
	...
	switch scenario {
	case "cgo_full":
		// Fully CGo driver (mattn/go-sqlite3)
		store, err = setupCGoStore(ctx, dbPath)
	case "hybrid_cgo":
		// Our hybrid model with CGo enabled
		store, err = setupHybridStore(ctx, dbPath, true)
	case "hybrid_purego":
...
// setupHybridStore creates our hybrid model store
func setupHybridStore(ctx context.Context, dbPath string, cgoEnabled bool) (collection.Store, error) {
	opts := collection.Options{
		EnableFTS:  true,
		EnableJSON: true,
	}

	// Only provide extensions if CGo is enabled (for hybrid_cgo scenario)
	var extensions []sqlite.ExtensionConfig
	if cgoEnabled {
		// Don't actually load extensions, just test the hybrid model
		// Extensions would be nil, so only pure Go connection opens
		extensions = nil
	}

	return sqlite.NewSqliteStore(ctx, dbPath, opts, extensions)
}
...
func applySchema(store collection.Store) error {
	...
	if err := store.ExecuteRaw(ctx, collection.DefaultSchema); err != nil {
...
func (s *cgoStore) ExecuteRaw(ctx context.Context, query string, args ...interface{}) error {
